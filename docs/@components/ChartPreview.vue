...
const version = ref(window.ucharts.version());
...
function openStackBlitz() {
	...
	stackBlitz.openProject({
		title: `${props.title} - ucharts@${version.value}`,
		description: props.description,
		template: 'javascript',
		dependencies: {
			ucharts: version.value,
		},
		files,
	});
}
...
function getCodePenParameters() {
	...
	const parameters = {
		...
		js: js.replace(/import\s+{(\s+[^}]*\s+)}\s+from\s+'ucharts'/, 'const {$1} = ucharts').replace(/import.*\'\.\/index\.css\'/, ''),
		js_external: `https://unpkg.com/ucharts@${version.value}/dist/ucharts.min.js`,
	};
	if (props.css) {
				...
				CallExpression(path) {
					if (t.isCallExpression(path.node) && t.isIdentifier(path.node.callee, { name: 'callback' })) {
						const postMessageFun = t.expressionStatement(
							t.callExpression(t.memberExpression(t.identifier('window'), t.identifier('postMessage')), [
...
onUnmounted(() => {
	if (!!props.chartId) {
		...
		if (window.ucharts) {
			window.ucharts.dispose(props.chartId);
		}
	}
...
<template>
	<div class="chart-preview">
		<template v-if="!!props.chartId">
			<div ref="chartContainer" class="content-item chart">
				<Loading v-if="loading" />
			</div>
			<div class="code-action-container" :class="{ hiddenCode: !showCode }">
				<form action="https://codesandbox.io/api/v1/sandboxes/define" method="POST" target="_blank">
					<input type="hidden" name="parameters" :value="getCodeSandboxParameters()" />
					<button type="submit">
				...
				<form action="https://codepen.io/pen/define" method="POST" target="_blank">
					<input type="hidden" name="data" :value="getCodePenParameters()" />
					<button type="submit">
						<Tooltip :tip="i18n('component_chart_preview_open_in_codepen', lang)">
							<svg width="20px" height="20px" viewBox="0 0 24 24">
								<path
									stroke="none"
									d="m21.66 8.264l-9.18-6.12a.88.88 0 0 0-.966 0l-9.146 6.12c-.225.129-.354.451-.354.676v6.087c0 .258.129.548.354.741l9.147 6.087a.88.88 0 0 0 .966 0l9.146-6.087c.226-.129.355-.45.355-.74V8.94c.032-.257-.097-.547-.323-.676m-8.793-3.8l6.731 4.509l-3.06 1.996l-3.703-2.512c.032 0 .032-3.993.032-3.993m-1.707 0v3.993L7.424 10.97L4.43 8.973zM3.753 10.55L5.878 12l-2.125 1.45zm7.407 8.985l-6.73-4.509l2.994-1.996l3.736 2.512zm.87-5.475L8.97 12l3.06-2.061L15.09 12zm.837 5.475v-3.993l3.736-2.512l2.995 1.996zm7.407-6.087L18.15 12l2.125-1.45z" />
							</svg>
						</Tooltip>
					</button>
				</form>
				<button @click="openStackBlitz()">
				...
				<button @click="showCode = !showCode">
					<Tooltip
						:tip="showCode ? i18n('component_chart_preview_hide_code', lang) : i18n('component_chart_preview_show_code', lang)">
		...
		<div v-if="showCode" class="content-item chart-preview-code">
			<button class="copy" :class="{ copied: copied }" @click="copyHandler()" />
			<div v-html="codeHtml" />
		</div>
	</div>
</template>
...
.code-action-container a,
.code-action-container button {
	padding: 0 6px;
}
...
.code-action-container a svg,
.code-action-container button svg {
...
.code-action-container a svg:hover,
.code-action-container button svg:hover {
	opacity: 1;
	transform: scale(1.2);
}
</style>
...
.chart-preview-code button.copy:hover,
.chart-preview-code button.copy.copied {
